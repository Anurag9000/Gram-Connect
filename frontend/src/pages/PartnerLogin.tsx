...
export default function PartnerLogin() {
  ...
  const [email, setEmail] = useState('partner@test.com');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (email !== 'partner@test.com') {
        throw new Error(`Invalid email for partner. Use 'partner@test.com'`);
      }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-emerald-50 to-white px-4">
      <div className="w-full max-w-md rounded-xl bg-white p-8 shadow-lg">
        <button onClick={() => navigate('/')} className="mb-4 flex items-center text-emerald-600 hover:text-emerald-700">
          {t('auth.back_to_home')}
        </button>
        <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100">
          <Briefcase className="text-emerald-600" size={40} />
        </div>
        <h2 className="mb-6 text-center text-3xl font-bold text-emerald-700">Partner login</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full rounded-lg bg-emerald-600 py-3 font-semibold text-white transition hover:bg-emerald-700 disabled:bg-gray-400">
            {loading ? t('common.loading') : 'Sign in as partner'}
          </button>
        </form>
      </div>
    </div>
